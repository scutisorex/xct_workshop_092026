...
- Regular image stacks (e.g., .tif files), often the output of a µCT scanner. These come as stacks of image files that are numbered and go together to form a 3D volume. This is what you get from me when I do a scan for you in the FMNH XCT lab.
- There are a couple ways to import this type of data into Slicer, and both have advantages and disadvantages.
	- First: Get yourself a little data set from [here](https://www.dropbox.com/scl/fo/zrntxx7xvvdcnqigjri3x/AMyXGZHCDZa29_EKANAG_oc?rlkey=v05k3xily6tud1ax4vjwn9j35&st=dzsnecye&dl=0). Any of the .zip files are good. You get to pick from among 2 species of mole, one moonrat, and one shrew, but you have to figure out which is which! :) Note that each specimen has a ScanSettings document with the scan resolution - you will need that for import.
